...
fun ScaleFretboardScreen(onBack: () -> Unit) {
    ...
    Column(
        ...
        Row(
            ...
            Box {
                OutlinedButton(onClick = { keyMenuExpanded = true }) {
                    Text(SCALE_NOTE_NAMES[selectedKey], color = Color.White, fontWeight = FontWeight.Bold)
                }
        ...
        // Fretboard
        val totalWidthDp = (TOTAL_FRETS * 60 * zoom + 20).dp
        Box(
            modifier = Modifier
                .weight(1f)
                .fillMaxWidth()
...
private fun DrawScope.drawFretboard(
    rootNote: Int,
    scale: Scale,
    noteDisplay: NoteDisplay,
    ...
    for (fret in 1..TOTAL_FRETS) {
        val x = nutWidth + (fret - 0.5f) * fretWidthPx
        drawContext.canvas.nativeCanvas.drawText("$fret", x, stringSpacing * 0.4f, textPaint)
    }
    ...
    for (string in 0 until 6) {
        ...
        for (fret in 0..TOTAL_FRETS) {
            ...
            val x = if (fret == 0) {
                ...
                nutWidth + (fret - 0.5f) * fretWidthPx
            }
            ...
            if (noteDisplay != NoteDisplay.NONE) {
                ...
                drawContext.canvas.nativeCanvas.drawText(label, x, y + 5f, notePaint)
            }
        }
    }
}
